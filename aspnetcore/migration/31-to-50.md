...
1. W pliku `wwwroot/index.html`:

   * Dodaj `<link>` element do `<head>` elementu dla stylów o określonym zakresie.
   * Zmień element, który ładuje `App` składnik.
...
1. W programie `Shared/MainLayout.razor` należy ująć całą zawartość w `<div>` elemencie z `id` `page` :
...
1. Najnowszy plik podstawowy `wwwroot/css/app.css` zawiera następujące style. Usuń dodatkowe style, pozostawiając następujące style i dodanych do aplikacji.

    `wwwroot/css/app.css` (style podstawowe, które nie obejmują stylów dodanych przez dewelopera):
