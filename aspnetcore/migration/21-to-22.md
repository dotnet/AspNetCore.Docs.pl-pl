...
Projekty ukierunkowane na platformę .NET Core powinny korzystać z [TFM](/dotnet/standard/frameworks) wersji nowszej niż lub równej platformie .net Core 2,2. W pliku projektu zaktualizuj tekst wewnętrzny `<TargetFramework>` węzła przy użyciu: `netcoreapp2.2`
...
* `<handlers>` W pozycji, która dodaje moduł ASP.NET Core (`name="aspNetCore"`), Zmień wartość `modules` atrybutu z `AspNetCoreModule` na. `AspNetCoreModuleV2`
...
Jeśli aplikacja nie `CreateDefaultBuilder` wywołuje i nie kompiluje hosta ręcznie w `Program` klasie, wywołaj <xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*> **przed** wywołaniem: `ConfigureKestrel`
