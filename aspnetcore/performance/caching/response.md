...
| [Pragm](https://tools.ietf.org/html/rfc7234#section-5.4)  | Istnieje w celu zachowania zgodności z poprzednimi wersjami z pamięcią `no-cache` podręczną protokołu HTTP/1.0 na potrzeby ustawienia zachowania. Jeśli `Cache-Control` nagłówek jest obecny, `Pragma` nagłówek jest ignorowany. |
...
[Specyfikacja buforowania HTTP 1,1 dla nagłówka Cache-Control](https://tools.ietf.org/html/rfc7234#section-5.2) wymaga, aby pamięć podręczna zatraktował `Cache-Control` prawidłowy nagłówek Wysłany przez klienta. Klient może wykonywać żądania z wartością `no-cache` nagłówka i wymusić, aby serwer generował nową odpowiedź dla każdego żądania.

Zawsze, gdy nagłówki `Cache-Control` żądań klientów są brane pod uwagę, Jeśli rozważasz cel buforowania http. Zgodnie z oficjalną specyfikacją buforowanie jest przeznaczone do zmniejszenia opóźnień i obciążenia sieci w celu zaspokojenia żądań w sieci klientów, serwerów proxy i serwerów. Nie musi to być sposób sterowania obciążeniem na serwerze źródłowym.
...
Ten nagłówek jest zapisywana tylko wtedy <xref:Microsoft.AspNetCore.Mvc.CacheProfile.VaryByHeader> , gdy właściwość jest ustawiona. Właściwość ustawiona na wartość `Vary` właściwości. Poniższy przykład używa <xref:Microsoft.AspNetCore.Mvc.CacheProfile.VaryByHeader> właściwości:
...
Jeśli <xref:Microsoft.AspNetCore.Mvc.CacheProfile.NoStore> jest `false` i <xref:Microsoft.AspNetCore.Mvc.CacheProfile.Location> ma `None`wartość `Cache-Control`, i `Pragma` są ustawione na `no-cache`.
...
<xref:Microsoft.AspNetCore.Mvc.CacheProfile.Location>Opcje `Any` `Client` i przetłumaczą odpowiednio `Cache-Control` wartości nagłówka `public` i. `private` Jak zostało to opisane w sekcji [NoStore and Location. None](#nostore-and-locationnone) , <xref:Microsoft.AspNetCore.Mvc.CacheProfile.Location> ustawienie `None` ustawia zarówno `Cache-Control` , `Pragma` jak i `no-cache`nagłówki.
...
Poniższy przykład przedstawia model strony Cache3 z przykładowej aplikacji oraz nagłówki utworzone przez ustawienie <xref:Microsoft.AspNetCore.Mvc.CacheProfile.Duration> i opuszczające wartość domyślną: <xref:Microsoft.AspNetCore.Mvc.CacheProfile.Location>
...
Zamiast duplikowania ustawień pamięci podręcznej odpowiedzi w wielu atrybutach akcji kontrolera, profile pamięci podręcznej można skonfigurować jako opcjeRazor podczas konfigurowania `Startup.ConfigureServices`MVC/Pages in. Wartości Znalezione w profilu pamięci podręcznej, do którego istnieje odwołanie, <xref:Microsoft.AspNetCore.Mvc.ResponseCacheAttribute> są używane jako wartości domyślne przez i są zastępowane przez wszystkie właściwości określone w atrybucie.
...
Model strony Cache4 aplikacji przykładowej odwołuje się `Default30` do profilu pamięci podręcznej:

[!code-csharp[](response/samples/2.x/ResponseCacheSample/Pages/Cache4.cshtml.cs?name=snippet)]

<xref:Microsoft.AspNetCore.Mvc.ResponseCacheAttribute> Można zastosować do:

* RazorAtrybuty obsługi stron (klasy) &ndash; nie mogą być stosowane do metod obsługi.
...
Otrzymany nagłówek zastosowany do odpowiedzi strony Cache4 przez profil `Default30` pamięci podręcznej:
