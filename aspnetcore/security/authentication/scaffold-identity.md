...
Poprzedzający kod oznacza kod, który jest duplikowany w *obszarach/ Identity / Identity HostingStartup.cs*

Zazwyczaj aplikacje, które zostały utworzone przy użyciu pojedynczych kont, powinny * **nie** _ utworzyć nowego kontekstu danych.
...
Identityjest skonfigurowany w _Areas/ Identity / Identity HostingStartup.cs *. Aby uzyskać więcej informacji, zobacz [IHostingStartup](xref:fundamentals/configuration/platform-specific-configuration).
...
Identityjest skonfigurowany w *obszarach/ Identity / Identity HostingStartup.cs* . Aby uzyskać więcej informacji, zobacz IHostingStartup.
...
Identityjest skonfigurowany w *obszarach/ Identity / Identity HostingStartup.cs* . Aby uzyskać więcej informacji, zobacz [IHostingStartup](xref:fundamentals/configuration/platform-specific-configuration).
...
## <a name="standalone-or-hosted-no-locblazor-webassembly-apps"></a>Aplikacje autonomiczne lub hostowane Blazor WebAssembly

Aplikacje po stronie klienta Blazor WebAssembly używają własnych Identity metod interfejsu użytkownika i nie mogą używać ASP.NET Core Identity szkieletów. Po stronie serwera ASP.NET Core aplikacje rozwiązań hostowanych Blazor mogą postępować zgodnie ze Razor wskazówkami dotyczącymi stron/MVC w tym artykule i są skonfigurowane tak jak każdy inny typ aplikacji ASP.NET Core, która obsługuje Identity .

BlazorStruktura nie obejmuje Razor wersji składników Identity stron interfejsu użytkownika. Identity Składniki interfejsu użytkownika Razor mogą być utworzone niestandardowo lub uzyskane z nieobsługiwanych źródeł innych firm.

Aby uzyskać więcej informacji, zobacz [ Blazor zabezpieczenia i Identity artykuły](xref:blazor/security/index).
...
## <a name="password-configuration"></a>Konfiguracja hasła

Jeśli <xref:Microsoft.AspNetCore.Identity.PasswordOptions> są skonfigurowane w programie `Startup.ConfigureServices` , konfiguracja [ `[StringLength]` atrybutu](xref:System.ComponentModel.DataAnnotations.StringLengthAttribute) może być wymagana dla `Password` właściwości na stronach szkieletowych Identity . `InputModel``Password`właściwości znajdują się w następujących plikach:
...
W `Configure` metodzie `Startup` klasy Wywołaj <xref:Microsoft.AspNetCore.Builder.AuthAppBuilderExtensions.UseAuthentication%2A> po `UseStaticFiles` :
...
Wywołaj <xref:Microsoft.AspNetCore.Builder.AuthAppBuilderExtensions.UseAuthentication%2A> po `UseStaticFiles` :
