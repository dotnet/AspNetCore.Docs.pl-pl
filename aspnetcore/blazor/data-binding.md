...
Użyj `@bind-{ATTRIBUTE}` składni `@bind-{ATTRIBUTE}:event` with, aby powiązać atrybuty elementu inne `value`niż. W poniższym przykładzie styl akapitu zostanie zaktualizowany po zmianie `_paragraphStyle` wartości:
