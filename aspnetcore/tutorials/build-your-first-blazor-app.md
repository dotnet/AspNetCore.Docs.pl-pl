...
1. Sprawdzanie implementacji `Counter` składnika w pliku *Counter. Razor* .
   ...
   *Pages/Counter. Razor*:
   ...
   Interfejs użytkownika `Counter` składnika jest definiowany przy użyciu języka HTML. Logika renderowania dynamicznego (na przykład pętle, warunkowe, wyrażenia) jest dodawana przy użyciu osadzonej składni języka C# o nazwie [Razor](xref:mvc/views/razor) . Logika renderowania w kodzie HTML i języka C# jest konwertowana na klasę składnika w czasie kompilacji. Nazwa wygenerowanej klasy .NET jest zgodna z nazwą pliku.
