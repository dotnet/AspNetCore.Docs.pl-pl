...
Jeśli korzystasz z pliku [Global. JSON](/dotnet/core/tools/global-json) , który będzie przeznaczony dla konkretnej wersji zestaw .NET Core SDK, zaktualizuj `version` Właściwość do zainstalowanej wersji zestawu SDK programu .NET 5,0. Przykład:
...
W pliku projektu należy zaktualizować atrybut "każdy" `Microsoft.AspNetCore.*` i " `Microsoft.Extensions.*` odwołanie do pakietu" `Version` do 5.0.0 lub nowszego. Przykład:
