...
* Zmień platformę docelową na .NET Core 2,1, aktualizując plik projektu `<TargetFramework>netcoreapp2.1</TargetFramework>`do.
* Zastąp odwołanie do pakietu `Microsoft.AspNetCore.All` dla elementu odwołaniem do `Microsoft.AspNetCore.App`pakietu. Może być konieczne dodanie zależności, które zostały usunięte z `Microsoft.AspNetCore.All`programu. Aby uzyskać więcej informacji, zobacz <xref:fundamentals/metapackage#migrate> i <xref:fundamentals/metapackage-app>.
* Usuń atrybut "Version" w odwołaniu do `Microsoft.AspNetCore.App`pakietu. Projekty, które `<Project Sdk="Microsoft.NET.Sdk.Web">` nie muszą ustawiać wersji. Wersja jest implikowana przez platformę docelową i wybierana do najlepszego dopasowania do sposobu działania ASP.NET Core 2,1. Więcej informacji można znaleźć w sekcji [reguły dotyczące projektów przeznaczonych dla platformy udostępnionej](#rules-for-projects-targeting-the-shared-framework) .
...
* Opcjonalne: można usunąć element ** &lt;DotNetCliToolReference&gt; ** dla `Microsoft.VisualStudio.Web.CodeGeneration.Tools`elementu. Możesz zamienić to narzędzie na wersję zainstalowaną globalnie, `dotnet tool install -g dotnet-aspnet-codegenerator`uruchamiając program.
...
* Wszystkie projekty, które muszą `Microsoft.AspNetCore.All` używać `Microsoft.AspNetCore.App` lub powinny dodać odwołanie do pakietu w pliku projektu, nawet jeśli zawierają odwołanie do projektu do innego projektu przy użyciu `Microsoft.AspNetCore.All` lub. `Microsoft.AspNetCore.App`

  Przykład:

  * `MyApp`zawiera odwołanie do `Microsoft.AspNetCore.App`pakietu.
  * `MyApp.Tests`ma odwołanie do `MyApp.csproj`projektu.
...
Poniższy kod przedstawia zmiany w 2,1 kodzie generowanym przez szablon. Wszystkie zmiany są nowo dodany kod, z wyjątkiem `UseBrowserLink` tego, że został usunięty:
...
* Wszelkie poprawki błędów lub nowe funkcje tożsamości są zawarte w [pakiecie Microsoft. AspNetCore. app](xref:fundamentals/metapackage-app). Aktualizacja zaktualizowanej tożsamości `Microsoft.AspNetCore.App` jest automatycznie aktualizowana.
...
Aplikacje, które mają kod Identity wykorzystujący i Identity zastępujący Interfejs Identity użytkownika 2,0 z biblioteką 2,1 Identity , muszą `/Identity` uwzględnić adresy URL z segmentami dołączonymi do identyfikatorów URI. Jednym ze sposobów obsługi nowych Identity punktów końcowych jest skonfigurowanie przekierowań, na przykład z `/Account/Login` do. `/Identity/Account/Login`
...
* Użyj kodu Identity interfejsu użytkownika 2,0 bez zmian. Korzystanie Identity z kodu interfejsu użytkownika 2,0 jest w pełni obsługiwane. Jest to dobre podejście, gdy wprowadzono znaczące zmiany w wygenerowanym Identity kodzie.
* Usuń istniejący Identity kod i [szkielet Identity ](xref:security/authentication/scaffold-identity) 2,0 do projektu. W projekcie zostanie użyta [ Razor Biblioteka klas](xref:razor-pages/ui-class) [ASP.NET Core Identity ](xref:security/authentication/identity) . Można wygenerować kod i interfejs użytkownika dla dowolnego ze zmodyfikowanego kodu Identity interfejsu użytkownika. Zastosuj zmiany kodu do nowo poszkieletowego kodu interfejsu użytkownika.
* Usuń istniejący Identity kod i [szkielet Identity ](xref:security/authentication/scaffold-identity) 2,0 w projekcie z opcją **zastąpienia wszystkich plików**.
...
* Usuń następujące foldery i wszystkie znajdujące się w nich pliki:
  ...
  * *Rozszerzenia*
* Skompiluj projekt.
* [Tworzenie Identity szkieletu](xref:security/authentication/scaffold-identity) w projekcie:
...
* Usuń klasę Identity pochodną wygenerowaną `IdentityDbContext` przez szkielet w folderze *Areas/Identity/Data/* .
* Usuń *obszary/Identity/IdentityHostingStartup.cs*.
* Aktualizowanie pliku * \_LoginPartial. cshtml* :
  ...
  * Zaktualizuj `<form />` element do `<form asp-area="Identity" asp-page="/Account/Logout" asp-route-returnUrl="@Url.Page("/Index", new { area = "" })" method="post" id="logoutForm" class="navbar-right">`elementu.

  Poniższy kod przedstawia zaktualizowany * \_plik LoginPartial. cshtml* :
...
## <a name="changes-to-razor-pages-projects-razor-files"></a>Zmiany w Razor plikach projektów Razor stron
...
## <a name="changes-to-mvc-projects-razor-files"></a>Zmiany w plikach projektów Razor MVC
...
* Metoda `Privacy` akcji jest dodawana do kontrolera macierzystego.
