...
[Zasoby dodatkowe](#additional-resources)  
...
### <a name="5000-in-process-handler-load-failure"></a>500,0 błąd ładowania procedury obsługi w procesie
...
### <a name="50030-in-process-startup-failure"></a>Niepowodzenie uruchamiania w procesie 500,30
...
1. Wyłącz publikowanie pojedynczych plików przez ustawienie właściwości `PublishSingleFile` MSBuild na. `false`
1. Przejdź do modelu hostingu poza procesem, ustawiając właściwość programu `AspNetCoreHostingModel` MSBuild na `OutOfProcess`wartość.
...
1. Ustaw **stdoutLogEnabled** wartość stdoutLogEnabled `true` na i zmień ścieżkę **stdoutLogFile** na: `\\?\%home%\LogFiles\stdout`.
...
1. Ustaw **stdoutLogEnabled** wartość stdoutLogEnabled `false`na.
...
* Usuń plik `<handlerSettings>` z pliku *Web. config* lokalnie i Wdróż ponownie aplikację.
...
Ustawienie zmiennej środowiskowej dla `ASPNETCORE_ENVIRONMENT` programu jest zalecane wyłącznie do użytku na przejściowych i testowych serwerach, które nie są dostępne w Internecie. Usuń zmienną środowiskową z pliku *Web. config* po rozwiązaniu problemu. Aby uzyskać informacje na temat ustawiania zmiennych środowiskowych w *pliku Web. config*, zobacz [EnvironmentVariables podrzędny element aspNetCore](xref:host-and-deploy/aspnet-core-module#setting-environment-variables).

### <a name="obtain-data-from-an-app"></a>Uzyskiwanie danych z aplikacji

Jeśli aplikacja może odpowiadać na żądania, uzyskiwać żądania, połączenia i dodatkowe dane z aplikacji przy użyciu wbudowanego oprogramowania terminala. Aby uzyskać więcej informacji i przykładowy kod, <xref:test/troubleshoot#obtain-data-from-an-app>Zobacz.
