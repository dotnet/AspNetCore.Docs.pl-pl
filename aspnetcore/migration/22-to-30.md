...
Jeśli Twoje rozwiązanie polega na pliku [Global. JSON](/dotnet/core/tools/global-json) , który będzie przeznaczony dla konkretnej wersji zestaw .NET Core SDK, zaktualizuj `version` jego właściwość do wersji 3,0 zainstalowanej na maszynie:
...
* IdentityINTERFEJSU użytkownika

  Obsługę [ Identity interfejsu użytkownika](xref:security/authentication/identity) można dodać, odwołując się do [Microsoft.IdentityAspNetCore... Pakiet interfejsu użytkownika](https://www.nuget.org/packages/Microsoft.AspNetCore.Identity.UI) .
...
* Obsługa formatowania i negocjowania zawartości `System.Net.HttpClient` &ndash; dla pakietu NuGet [Microsoft. ASPNET. WebApi. Client](https://www.nuget.org/packages/Microsoft.AspNet.WebApi.Client/) oferuje przydatną rozszerzalność `System.Net.HttpClient` z użyciem interfejsów `ReadAsAsync` API `PostJsonAsync`, takich jak i.

* RazorObsługa kompilacji &ndash; środowiska uruchomieniowego dla kompilacji Razor widoków i stron jest teraz częścią [Microsoft. AspNetCore. MVC.Razor. RuntimeCompilation](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.Razor.RuntimeCompilation).

* Obsługa `Newtonsoft.Json` platformy mvc (JSON.NET &ndash; ) obsługuje korzystanie z funkcji `Newtonsoft.Json` MVC with jest teraz częścią [Microsoft. AspNetCore. MVC. NewtonsoftJson](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.NewtonsoftJson).

## <a name="startup-changes"></a>Zmiany uruchamiania

Na poniższej ilustracji przedstawiono usunięte i zmienione wiersze w aplikacji sieci Web ASP.NET Core Razor 2,2:

![usuniętych i zmienionych wierszy w aplikacji internetowej Razor ASP.NET Core 2,2](22-to-30/_static/startup2.2.png)
...
Na poniższej ilustracji przedstawiono dodane i zmienione wiersze w aplikacji sieci Web ASP.NET Core Razor 3,0:

![dodane i zmienione wiersze w aplikacji internetowej ASP.NET Core 3,0 Razor](22-to-30/_static/startup3.0.png)
...
* `app.UseEndpoints`, zobacz [ Razor strony](#razor-pages) lub [Migrowanie uruchamiania. Skonfiguruj](#migrate-startupconfigure) w tym dokumencie.
...
### <a name="razor-class-library"></a>RazorBiblioteka klas

RazorProjekty biblioteki klas, które zapewniają składniki interfejsu użytkownika dla MVC, `AddRazorSupportForMvc` muszą ustawić właściwość w pliku projektu:
...
Migruj konfigurację Kestrel do [konstruktora hosta sieci Web](#hostb) dostarczonego `ConfigureWebHostDefaults` przez program (*program.cs*):
...
Jeśli aplikacja tworzy hosta ręcznie przy użyciu `HostBuilder`programu, należy `UseKestrel` wywołać konstruktora hosta sieci Web w `ConfigureWebHostDefaults`:
...
* `GetDeclaredTrailers`&ndash; Pobiera nagłówek żądania `Trailer` , który zawiera listę przyczep, które mają być oczekiwane po treści.
...
Domyślny serializator JSON dla ASP.NET Core jest teraz <xref:System.Text.Json>nowy w programie .net Core 3,0. Rozważ użycie `System.Text.Json` , gdy jest to możliwe. Jest wysoka wydajność i nie wymaga dodatkowej zależności biblioteki. Jednak od momentu `System.Text.Json` , gdy jest to nowe, może to oznaczać brak funkcji wymaganych przez aplikację. Aby uzyskać więcej informacji, zobacz [Migrowanie z Newtonsoft. JSON do System. Text. JSON](/dotnet/standard/serialization/system-text-json-migrate-from-newtonsoft-how-to).

### <a name="use-newtonsoftjson-in-an-aspnet-core-30-signalr-project"></a>Używanie Newtonsoft. JSON w projekcie ASP.NET Core 3,0 SignalR
...
* Na kliencie należy utworzyć `AddNewtonsoftJsonProtocol` łańcuch wywołań metody do `HubConnectionBuilder` wystąpienia:
...
* Na serwerze należy utworzyć `AddNewtonsoftJsonProtocol` łańcuch wywołania metody wywołania `AddSignalR` metody w: `Startup.ConfigureServices`
...
Dostępne `IServiceCollection` są trzy nowe metody rozszerzenia najwyższego poziomu związane z scenariuszami MVC. Szablony wykorzystują te nowe metody zamiast `AddMvc`. Jednak nadal `AddMvc` zachowuje się tak, jak w poprzednich wersjach.
...
Routing punktów końcowych obsługuje tę samą składnię wzorca trasy i funkcje tworzenia `IRouter`wzorców tras. Obsługa `IRouteConstraint`routingu punktów końcowych. Routing punktów `[Route]`końcowych `[HttpGet]`obsługuje, i inne atrybuty routingu MVC.
...
* Jeśli aplikacja używa scenariuszy [CORS](xref:security/cors) `[EnableCors]`, takich jak, należy umieścić wywołanie `UseCors` przed dowolnym innym oprogramowanie pośredniczące, które używa mechanizmu CORS (na przykład Umieść `UseCors` przed `UseAuthentication`, `UseAuthorization`, i `UseEndpoints`).
...
> [!WARNING]
> W przypadku większości `UseAuthentication`aplikacji wywołania, `UseAuthorization`i `UseCors` muszą znajdować się między wywołaniami `UseRouting` i `UseEndpoints` , aby obowiązywać.
...
Ponadto każdy punkt końcowy może dostosować wymagania dotyczące autoryzacji. W poniższym przykładzie `UseAuthorization` program przetwarza autoryzację przy użyciu `DefaultPolicy`, ale punkt `/healthz` końcowy sprawdzania kondycji wymaga `admin` użytkownika:
...
Mapowanie SignalR centrów odbywa się teraz w `UseEndpoints`obrębie.

Mapuj każde centrum za `MapHub`pomocą. Tak jak w poprzednich wersjach, każde centrum jest jawnie wymienione.

W poniższym przykładzie dodano obsługę `ChatHub` SignalR centrum:
...
Ponieważ Routing obejmuje teraz obsługę więcej niż tylko MVC, terminologia zmieniła się w taki sposób, że te metody jasno określają to, co robią. Trasy konwencjonalne, takie `MapControllerRoute` / `MapAreaControllerRoute` / `MapDefaultControllerRoute` jak są stosowane w kolejności, w jakiej zostały dodane. Najpierw umieść bardziej szczegółowe trasy (takie jak trasy dla obszaru).
...
W ASP.NET Core 3,0 ASP.NET Core MVC usuwa `Async` sufiks z nazw akcji kontrolera. Ta nowa wartość domyślna ma wpływ na generowanie routingu i linków. Przykład:
...
* Wymagana jest `Async` generacja linku określająca sufiks. Przykład:
...
* Generowanie łącza nie wymaga określenia `Async` sufiksu. Przykład:

    ```cshtml
    <a asp-controller="Products" asp-action="List">List</a>
    ```

Ta zmiana nie ma wpływu na nazwy określone [`[ActionName]`](/dotnet/api/microsoft.aspnetcore.mvc.actionnameattribute) przy użyciu atrybutu. Zachowanie domyślne można wyłączyć przy użyciu następującego kodu w programie `Startup.ConfigureServices`:
...
Zgodnie z opisem w dokumentacji dotyczącej [różnic między wcześniejszymi wersjami routingu](xref:fundamentals/routing#differences-from-earlier-versions-of-routing)istnieją pewne różnice w generowaniu linków ( `Url.Link` na przykład przy użyciu i podobne interfejsy API). Należą do nich następujące elementy:
...
Na Razor stronach mapowania teraz odbywa się `UseEndpoints`wewnątrz.
...
Najbardziej znacząca zmiana z `WebHostBuilder` do `HostBuilder` na jest [iniekcją zależności (di)](xref:fundamentals/dependency-injection). W przypadku `HostBuilder`korzystania z programu można wstrzyknąć tylko następujące elementy `Startup`do konstruktora:
...
## <a name="identity-ui"></a>IdentityINTERFEJSU użytkownika
...
* Aplikacje, które nie Razor korzystają ze stron `MapRazorPages`, muszą wywoływać. Zobacz [ Razor strony](#razor-pages) w tym dokumencie.
* Uruchomienie Bootstrap 4 jest domyślną strukturą interfejsu użytkownika. Ustaw właściwość `IdentityUIFrameworkVersion` projektu w celu zmiany wartości domyślnej. Aby uzyskać więcej informacji, zobacz [ten anons usługi GitHub](https://github.com/aspnet/Announcements/issues/380).

## SignalR

Klient SignalR JavaScript zmienił się z `@aspnet/signalr` na. `@microsoft/signalr` Aby reagować na tę zmianę, Zmień odwołania w plikach *Package. JSON* , `require` instrukcje i instrukcje języka ECMAScript `import` .
...
Jeśli używasz [funkcji Newtonsoft. JSON, które nie są obsługiwane w pliku System. Text. JSON](/dotnet/standard/serialization/system-text-json-migrate-from-newtonsoft-how-to), możesz przełączyć się z powrotem `Newtonsoft.Json`do. Zobacz [Używanie Newtonsoft. JSON w projekcie ASP.NET Core 3,0 SignalR ](#use-newtonsoftjson-in-an-aspnet-core-30-signalr-project) wcześniej w tym artykule.
...
Aby uzyskać więcej informacji Razor na temat kompilowania <xref:mvc/views/view-compilation>plików, zobacz.
...
Aby uzyskać więcej informacji na temat używania interfejsów API ASP.NET Core w bibliotece klas <xref:fundamentals/target-aspnetcore>, zobacz.

## <a name="miscellaneous-changes"></a>Różne zmiany

System sprawdzania poprawności w programie .NET Core 3,0 lub nowszy traktuje parametry niedopuszczające wartości null lub właściwości powiązane tak `[Required]` , jakby miały atrybut. Aby uzyskać więcej informacji, zobacz [[Required] Attribute](xref:mvc/models/validation?view=aspnetcore-3.1#required-attribute).
...
W przypadku aplikacji, <xref:Microsoft.AspNetCore.TestHost.TestServer> które używają bezpośrednio z [hostem ogólnym](xref:fundamentals/host/web-host), `TestServer` Utwórz <xref:Microsoft.AspNetCore.Hosting.IWebHostBuilder> za <xref:Microsoft.Extensions.Hosting.GenericHostWebHostBuilderExtensions.ConfigureWebHost%2A>pomocą w:
