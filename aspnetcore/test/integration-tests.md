...
* [Testy jednostkowe Razor Pages](xref:test/razor-pages-tests)
...
1. Dziedzicz z `WebApplicationFactory` i Zastąp [ConfigureWebHost](/dotnet/api/microsoft.aspnetcore.mvc.testing.webapplicationfactory-1.configurewebhost). [IWebHostBuilder](/dotnet/api/microsoft.aspnetcore.hosting.iwebhostbuilder) umożliwia konfigurację kolekcji usług z [ConfigureServices](/dotnet/api/microsoft.aspnetcore.hosting.istartup.configureservices):
   ...
   Wypełnianie bazy danych w [aplikacji przykładowej](https://github.com/dotnet/AspNetCore.Docs/tree/master/aspnetcore/test/integration-tests/samples) jest wykonywane `InitializeDbForTests` przez metodę. Metoda jest opisana w [przykładowej testów integracji: sekcja testowa aplikacja w organizacji](#test-app-organization) .

   Kontekst bazy danych SUT jest zarejestrowany w `Startup.ConfigureServices` metodzie. Wywołanie zwrotne aplikacji testowej jest wykonywane po wykonaniu `Startup.ConfigureServices` kodu aplikacji. *after* `builder.ConfigureServices` Kolejność wykonywania jest istotną zmianą dla [hosta ogólnego](xref:fundamentals/host/generic-host) w wersji ASP.NET Core 3,0. Aby użyć innej bazy danych dla testów niż baza danych aplikacji, należy zastąpić kontekst bazy danych aplikacji `builder.ConfigureServices`.
...
3. Typowy test używa metod `HttpClient` i, aby przetworzyć żądanie i odpowiedź:
...
* Wartość `Location` nagłówka w nagłówkach odpowiedzi jest sprawdzana w celu potwierdzenia, że zaczyna `http://localhost/Identity/Account/Login`się od, a nie od końcowej odpowiedzi na `Location` stronę logowania, gdzie nagłówek nie może być obecny.
...
`TestAuthHandler` Jest wywoływana w celu uwierzytelnienia użytkownika, gdy w schemacie uwierzytelniania jest `Test` ustawiona `AddAuthentication` wartość WHERE jest `ConfigureTestServices`zarejestrowana dla:
...
* Ustaw zmienną `ASPNETCORE_ENVIRONMENT` środowiskową (na przykład `Staging` `Production`, lub inną wartość niestandardową, taką jak `Testing`).
...
`WebApplicationFactory` Konstruktor wnioskuje ścieżkę [katalogu głównego zawartości](xref:fundamentals/index#content-root) aplikacji, wyszukując [WebApplicationFactoryContentRootAttribute](/dotnet/api/microsoft.aspnetcore.mvc.testing.webapplicationfactorycontentrootattribute) na zestawie zawierającym testy integracji z kluczem równym `TEntryPoint` zestawowi. `System.Reflection.Assembly.FullName` W przypadku nieznalezienia `WebApplicationFactory` atrybutu o poprawnym kluczu Wróć do wyszukiwania pliku rozwiązania (*. sln*) i dołączaj nazwę `TEntryPoint` zestawu do katalogu rozwiązania. Katalog główny aplikacji (ścieżka katalogu głównego zawartości) służy do odnajdywania widoków i plików zawartości.
...
* `Message` Komunikat jest opisywany przez klasę (*Data/Message. cs*) z dwiema właściwościami: `Id` (Key) i `Text` (Message). `Text` Właściwość jest wymagana i jest ograniczona do 200 znaków.
...
Platforma testowa jest [xUnit](https://xunit.github.io/). Testy integracji są przeprowadzane przy użyciu [programu Microsoft. AspNetCore. TestHost](/dotnet/api/microsoft.aspnetcore.testhost), który obejmuje [TestServer](/dotnet/api/microsoft.aspnetcore.testhost.testserver). Ponieważ pakiet [Microsoft. AspNetCore. MVC. test](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.Testing) jest używany do konfigurowania hosta testowego i serwera testowego, pakiety `TestHost` i `TestServer` nie wymagają bezpośrednich odwołań do pakietów w pliku projektu aplikacji testowej ani w konfiguracji dewelopera w aplikacji testowej.
...
Kontekst bazy danych SUT jest zarejestrowany w `Startup.ConfigureServices` metodzie. Wywołanie zwrotne aplikacji testowej jest wykonywane po wykonaniu `Startup.ConfigureServices` kodu aplikacji. *after* `builder.ConfigureServices` Aby użyć innej bazy danych dla testów, kontekst bazy danych aplikacji musi zostać zastąpiony `builder.ConfigureServices`. Aby uzyskać więcej informacji, zobacz sekcję [Dostosowywanie WebApplicationFactory](#customize-webapplicationfactory) .
