...
---
# <a name="aspnet-core-no-locblazor-forms-and-validation"></a><span data-ttu-id="931e6-103">ASP.NET Core :::no-loc(Blazor)::: formularzy i walidacji</span><span class="sxs-lookup"><span data-stu-id="931e6-103">ASP.NET Core :::no-loc(Blazor)::: forms and validation</span></span>
...
<span data-ttu-id="931e6-105">Formularze i walidacje są obsługiwane w przypadku :::no-loc(Blazor)::: używania [adnotacji danych](xref:mvc/models/validation).</span><span class="sxs-lookup"><span data-stu-id="931e6-105">Forms and validation are supported in :::no-loc(Blazor)::: using [data annotations](xref:mvc/models/validation).</span></span>
...
* <span data-ttu-id="931e6-204">Rozwiązanie hostowane :::no-loc(Blazor)::: utworzone przez [ :::no-loc(Blazor)::: szablon hostowanego projektu](xref:blazor/hosting-models#blazor-webassembly).</span><span class="sxs-lookup"><span data-stu-id="931e6-204">A hosted :::no-loc(Blazor)::: solution created by the [:::no-loc(Blazor)::: Hosted project template](xref:blazor/hosting-models#blazor-webassembly).</span></span> <span data-ttu-id="931e6-205">Przykład może być używany z dowolnym bezpiecznymi :::no-loc(Blazor)::: rozwiązaniami obsługiwanymi opisanymi w temacie [zabezpieczenia i :::no-loc(Identity)::: Dokumentacja](xref:blazor/security/webassembly/index#implementation-guidance).</span><span class="sxs-lookup"><span data-stu-id="931e6-205">The example can be used with any of the secure hosted :::no-loc(Blazor)::: solutions described in the [Security and :::no-loc(Identity)::: documentation](xref:blazor/security/webassembly/index#implementation-guidance).</span></span>
...
* <span data-ttu-id="931e6-207">:::no-loc(Blazor)::: <xref:Microsoft.AspNetCore.Components.Forms.DataAnnotationsValidator> Składnik struktury.</span><span class="sxs-lookup"><span data-stu-id="931e6-207">The :::no-loc(Blazor)::: framework's <xref:Microsoft.AspNetCore.Components.Forms.DataAnnotationsValidator> component.</span></span>
...
> [!NOTE]
> <span data-ttu-id="931e6-232">Podejście do walidacji po stronie serwera w tej sekcji jest odpowiednie dla dowolnego z :::no-loc(Blazor WebAssembly)::: przykładów obsługiwanych rozwiązań w tym zestawie dokumentacji:</span><span class="sxs-lookup"><span data-stu-id="931e6-232">The server-side validation approach in this section is suitable for any of the :::no-loc(Blazor WebAssembly)::: hosted solution examples in this documentation set:</span></span>
>
...
> * [<span data-ttu-id="931e6-235">:::no-loc(Identity)::: Server</span><span class="sxs-lookup"><span data-stu-id="931e6-235">:::no-loc(Identity)::: Server</span></span>](xref:blazor/security/webassembly/hosted-with-identity-server)
...
<span data-ttu-id="931e6-264">:::no-loc(Blazor):::Platforma nie próbuje pominąć zachowania domyślnego, ponieważ spowodowałoby to:</span><span class="sxs-lookup"><span data-stu-id="931e6-264">The :::no-loc(Blazor)::: framework doesn't attempt to suppress the default behavior because it would involve:</span></span>
...
<span data-ttu-id="931e6-267">Najbardziej wiarygodny `null` odpowiednik w kodzie HTML jest *ciągiem pustym* `value` .</span><span class="sxs-lookup"><span data-stu-id="931e6-267">The most plausible `null` equivalent in HTML is an *empty string* `value`.</span></span> <span data-ttu-id="931e6-268">:::no-loc(Blazor):::Platforma obsługuje `null` konwersje do pustych ciągów dla dwukierunkowego powiązania z `<select>` wartością.</span><span class="sxs-lookup"><span data-stu-id="931e6-268">The :::no-loc(Blazor)::: framework handles `null` to empty string conversions for two-way binding to a `<select>`'s value.</span></span>

::: moniker-end

::: moniker range="< aspnetcore-5.0"

<span data-ttu-id="931e6-269">:::no-loc(Blazor):::Struktura nie jest automatycznie obsługiwana `null` dla pustych konwersji ciągów podczas próby dwukierunkowego powiązania z `<select>` wartością.</span><span class="sxs-lookup"><span data-stu-id="931e6-269">The :::no-loc(Blazor)::: framework doesn't automatically handle `null` to empty string conversions when attempting two-way binding to a `<select>`'s value.</span></span> <span data-ttu-id="931e6-270">Aby uzyskać więcej informacji, zobacz temat [Usuwanie powiązania `<select>` do wartości null (dotnet/aspnetcore #23221)](https://github.com/dotnet/aspnetcore/pull/23221).</span><span class="sxs-lookup"><span data-stu-id="931e6-270">For more information, see [Fix binding `<select>` to a null value (dotnet/aspnetcore #23221)](https://github.com/dotnet/aspnetcore/pull/23221).</span></span>
...
<span data-ttu-id="931e6-278">:::no-loc(Blazor)::: wykonuje dwa typy walidacji:</span><span class="sxs-lookup"><span data-stu-id="931e6-278">:::no-loc(Blazor)::: performs two types of validation:</span></span>
...
### <a name="no-locblazor-data-annotations-validation-package"></a><span data-ttu-id="931e6-300">:::no-loc(Blazor)::: Pakiet weryfikacji adnotacji danych</span><span class="sxs-lookup"><span data-stu-id="931e6-300">:::no-loc(Blazor)::: data annotations validation package</span></span>
