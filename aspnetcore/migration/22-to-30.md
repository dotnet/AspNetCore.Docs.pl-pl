...
Jeśli Twoje rozwiązanie polega na pliku [Global. JSON](/dotnet/core/tools/global-json) , który będzie przeznaczony dla konkretnej wersji zestaw .NET Core SDK, zaktualizuj jego `version` właściwość do wersji 3,0 zainstalowanej na maszynie:
...
* RazorKompilacja środowiska uruchomieniowego: obsługa kompilacji widoków i stron przez środowisko uruchomieniowe Razor jest teraz częścią [Microsoft. AspNetCore. MVC. Razor . RuntimeCompilation](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.Razor.RuntimeCompilation).
...
![usuniętych i zmienionych wierszy w Razor aplikacji internetowej ASP.NET Core 2,2](22-to-30/_static/startup2.2.png)
...
RazorProjekty biblioteki klas, które zapewniają składniki interfejsu użytkownika dla MVC, muszą ustawić `AddRazorSupportForMvc` Właściwość w pliku projektu:
...
* Wymagana jest generacja linku określająca `Async` sufiks. Przykład:
...
SignalRKlient JavaScript zmienił `@aspnet/signalr` się z na `@microsoft/signalr` . Aby reagować na tę zmianę, Zmień odwołania w plikach *Package. JSON* , `require` instrukcje i `import` instrukcje języka ECMAScript.
...
Jeśli używasz [funkcji Newtonsoft. JSON, które nie są obsługiwane w pliku System. Text. JSON](/dotnet/standard/serialization/system-text-json-migrate-from-newtonsoft-how-to), możesz przełączyć się z powrotem do `Newtonsoft.Json` . Zobacz [Używanie Newtonsoft. JSON w SignalR projekcie ASP.NET Core 3,0](#use-newtonsoftjson-in-an-aspnet-core-30-signalr-project) wcześniej w tym artykule.
