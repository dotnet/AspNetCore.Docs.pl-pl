...
*Wskazówki zawarte w tej sekcji dotyczą obu Blazor modeli hostingu. Poniższe sekcje zawierają dodatkowe wskazówki dotyczące hostingu modeli i typów aplikacji. Zastosuj guidnace ze wszystkich odpowiednich sekcji do aplikacji.*
...
1. W `wwwroot/index.html` Blazor WebAssembly aplikacji lub `Pages/_Host.cshtml` Blazor Server aplikacji, Dodaj `<link>` element do `<head>` elementu style. W poniższych `<link>` `href` wartości atrybutów elementu symbol zastępczy `{ASSEMBLY NAME}` jest nazwą zestawu aplikacji.
    ...
    Przykład autonomiczny Blazor WebAssembly :
...
1. W `Shared/MainLayout.razor` pliku Umieść całą zawartość w `<div>` elemencie z `class` `page` :
...
1. W programie `wwwroot/index.html` Zmień element, który ładuje `App` składnik:
