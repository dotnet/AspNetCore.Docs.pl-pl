...
Są to "Trzyelementowy". Pierwszy z nich określa, że jeśli `sortOrder` parametr ma wartość null lub jest pusty, NameSortParm powinna być ustawiona na "name_desc"; w przeciwnym razie należy ustawić pusty ciąg. Te dwie instrukcje umożliwiają widokowi ustawienie hiperłączy nagłówka kolumny w następujący sposób:
...
`CreateAsync` Metoda jest używana zamiast konstruktora do utworzenia obiektu, `PaginatedList<T>` ponieważ konstruktory nie mogą uruchomić kodu asynchronicznego.
...
Instrukcja LINQ grupuje jednostki studenta według daty rejestracji, oblicza liczbę jednostek w każdej grupie i zapisuje wyniki w kolekcji obiektów modelu `EnrollmentDateGroup` widoku.
