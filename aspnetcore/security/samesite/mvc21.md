...
W poprzednim kodzie zarówno uwierzytelnianie plików cookie, jak i stan sesji ustawiają ich atrybut sameSite `None`na, emitujący atrybut o `None` wartości, a także ustawić atrybut Secure na true.
...
Aby wstawić go do użycia `app.UseCookiePolicy()` potoku w `Configure(IApplicationBuilder, IHostingEnvironment)` metodzie w [Startup.cs](https://github.com/blowdart/AspNetSameSiteSamples/blob/master/AspNetCore21MVC/Startup.cs). Przykład:
