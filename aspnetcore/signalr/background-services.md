---
title: 'ASP.NET Core hosta :::no-loc(SignalR)::: w usługach w tle'
author: bradygaster
description: 'Dowiedz się, jak wysyłać komunikaty do :::no-loc(SignalR)::: klientów z klas BackgroundService platformy .NET Core.'
...
---
# <a name="host-aspnet-core-no-locsignalr-in-background-services"></a><span data-ttu-id="b1c46-103">ASP.NET Core hosta :::no-loc(SignalR)::: w usługach w tle</span><span class="sxs-lookup"><span data-stu-id="b1c46-103">Host ASP.NET Core :::no-loc(SignalR)::: in background services</span></span>
...
* <span data-ttu-id="b1c46-106">Hosting :::no-loc(SignalR)::: centrów przy użyciu procesu roboczego w tle hostowanego z ASP.NET Core.</span><span class="sxs-lookup"><span data-stu-id="b1c46-106">Hosting :::no-loc(SignalR)::: Hubs using a background worker process hosted with ASP.NET Core.</span></span>
...
## <a name="enable-no-locsignalr-in-startup"></a><span data-ttu-id="b1c46-110">Włącz :::no-loc(SignalR)::: przy uruchamianiu</span><span class="sxs-lookup"><span data-stu-id="b1c46-110">Enable :::no-loc(SignalR)::: in startup</span></span>

::: moniker range=">= aspnetcore-3.0"

<span data-ttu-id="b1c46-111">Hostowanie ASP.NET Core :::no-loc(SignalR)::: Hub w kontekście procesu roboczego w tle jest takie samo, jak hostowanie centrum w aplikacji sieci web ASP.NET Core.</span><span class="sxs-lookup"><span data-stu-id="b1c46-111">Hosting ASP.NET Core :::no-loc(SignalR)::: Hubs in the context of a background worker process is identical to hosting a Hub in an ASP.NET Core web app.</span></span> <span data-ttu-id="b1c46-112">W `Startup.ConfigureServices` metodzie wywoływanie `services.Add:::no-loc(SignalR):::` powoduje dodanie wymaganych usług do warstwy iniekcja (ASP.NET Core zależność) do obsługi :::no-loc(SignalR)::: .</span><span class="sxs-lookup"><span data-stu-id="b1c46-112">In the `Startup.ConfigureServices` method, calling `services.Add:::no-loc(SignalR):::` adds the required services to the ASP.NET Core Dependency Injection (DI) layer to support :::no-loc(SignalR):::.</span></span> <span data-ttu-id="b1c46-113">W programie `Startup.Configure` `MapHub` Metoda jest wywoływana w `UseEndpoints` wywołaniu zwrotnym, aby połączyć punkty końcowe centrum w potoku żądania ASP.NET Core.</span><span class="sxs-lookup"><span data-stu-id="b1c46-113">In `Startup.Configure`, the `MapHub` method is called in the `UseEndpoints` callback to connect the Hub endpoints in the ASP.NET Core request pipeline.</span></span>
...
<span data-ttu-id="b1c46-114">Hostowanie ASP.NET Core :::no-loc(SignalR)::: Hub w kontekście procesu roboczego w tle jest takie samo, jak hostowanie centrum w aplikacji sieci web ASP.NET Core.</span><span class="sxs-lookup"><span data-stu-id="b1c46-114">Hosting ASP.NET Core :::no-loc(SignalR)::: Hubs in the context of a background worker process is identical to hosting a Hub in an ASP.NET Core web app.</span></span> <span data-ttu-id="b1c46-115">W `Startup.ConfigureServices` metodzie wywoływanie `services.Add:::no-loc(SignalR):::` powoduje dodanie wymaganych usług do warstwy iniekcja (ASP.NET Core zależność) do obsługi :::no-loc(SignalR)::: .</span><span class="sxs-lookup"><span data-stu-id="b1c46-115">In the `Startup.ConfigureServices` method, calling `services.Add:::no-loc(SignalR):::` adds the required services to the ASP.NET Core Dependency Injection (DI) layer to support :::no-loc(SignalR):::.</span></span> <span data-ttu-id="b1c46-116">W programie `Startup.Configure` `Use:::no-loc(SignalR):::` Metoda jest wywoływana, aby połączyć punkty końcowe centrum w potoku żądania ASP.NET Core.</span><span class="sxs-lookup"><span data-stu-id="b1c46-116">In `Startup.Configure`, the `Use:::no-loc(SignalR):::` method is called to connect the Hub endpoint(s) in the ASP.NET Core request pipeline.</span></span>
...
<span data-ttu-id="b1c46-119">Aby uzyskać więcej informacji na temat silnych typach centrów, zobacz [Używanie centrów w :::no-loc(SignalR)::: dla ASP.NET Core](xref:signalr/hubs#strongly-typed-hubs).</span><span class="sxs-lookup"><span data-stu-id="b1c46-119">For more information on strongly typed Hubs, see [Use hubs in :::no-loc(SignalR)::: for ASP.NET Core](xref:signalr/hubs#strongly-typed-hubs).</span></span>

> [!NOTE]
> <span data-ttu-id="b1c46-120">Ta funkcja nie jest ograniczona do [klasy \<T> centrum](xref:Microsoft.AspNetCore.:::no-loc(SignalR):::.Hub`1) .</span><span class="sxs-lookup"><span data-stu-id="b1c46-120">This functionality isn't limited to the [Hub\<T>](xref:Microsoft.AspNetCore.:::no-loc(SignalR):::.Hub`1) class.</span></span> <span data-ttu-id="b1c46-121">Każda klasa, która dziedziczy z [koncentratora](xref:Microsoft.AspNetCore.:::no-loc(SignalR):::.Hub), na przykład [DynamicHub](xref:Microsoft.AspNetCore.:::no-loc(SignalR):::.DynamicHub), działa.</span><span class="sxs-lookup"><span data-stu-id="b1c46-121">Any class that inherits from [Hub](xref:Microsoft.AspNetCore.:::no-loc(SignalR):::.Hub), such as [DynamicHub](xref:Microsoft.AspNetCore.:::no-loc(SignalR):::.DynamicHub), works.</span></span>
...
## <a name="call-a-no-locsignalr-hub-from-a-background-service"></a><span data-ttu-id="b1c46-123">Wywoływanie :::no-loc(SignalR)::: centrum z poziomu usługi w tle</span><span class="sxs-lookup"><span data-stu-id="b1c46-123">Call a :::no-loc(SignalR)::: Hub from a background service</span></span>
...
## <a name="react-to-no-locsignalr-events-with-background-services"></a><span data-ttu-id="b1c46-128">Reagowanie na :::no-loc(SignalR)::: zdarzenia za pomocą usług w tle</span><span class="sxs-lookup"><span data-stu-id="b1c46-128">React to :::no-loc(SignalR)::: events with background services</span></span>

<span data-ttu-id="b1c46-129">Podobnie jak w przypadku aplikacji jednostronicowej korzystającej z klienta języka JavaScript dla programu :::no-loc(SignalR)::: lub aplikacji klasycznej platformy .NET, można <xref:signalr/dotnet-client> również użyć funkcji, a `BackgroundService` lub `IHostedService` do łączenia się z :::no-loc(SignalR)::: centrami i odpowiadać na zdarzenia.</span><span class="sxs-lookup"><span data-stu-id="b1c46-129">Like a Single Page App using the JavaScript client for :::no-loc(SignalR)::: or a .NET desktop app can do using the using the <xref:signalr/dotnet-client>, a `BackgroundService` or `IHostedService` implementation can also be used to connect to :::no-loc(SignalR)::: Hubs and respond to events.</span></span>
