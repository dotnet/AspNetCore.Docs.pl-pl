...
<span data-ttu-id="fffef-105">Izolacja CSS upraszcza rozmiary CSS aplikacji, uniemożliwiając zależności od stylów globalnych i pomaga uniknąć konfliktów stylów między składnikami i bibliotekami.</span><span class="sxs-lookup"><span data-stu-id="fffef-105">CSS isolation simplifies an app's CSS footprint by preventing dependencies on global styles and helps to avoid styling conflicts among components and libraries.</span></span>

## <a name="enable-css-isolation"></a><span data-ttu-id="fffef-106">Włącz izolację CSS</span><span class="sxs-lookup"><span data-stu-id="fffef-106">Enable CSS isolation</span></span> 

<span data-ttu-id="fffef-107">Aby zdefiniować Style specyficzne dla składnika, należy utworzyć `.razor.css` plik pasujący do nazwy `.razor` pliku składnika.</span><span class="sxs-lookup"><span data-stu-id="fffef-107">To define component-specific styles, create a `.razor.css` file matching the name of the `.razor` file for the component.</span></span> <span data-ttu-id="fffef-108">Ten `.razor.css` plik jest *plikiem CSS z zakresem*.</span><span class="sxs-lookup"><span data-stu-id="fffef-108">This `.razor.css` file is a *scoped CSS file*.</span></span> 
...
<span data-ttu-id="fffef-125">`MyProjectName.styles.css`Plik używa identyfikatora zakresu do grupowania deklaracji stylu ze składnikiem.</span><span class="sxs-lookup"><span data-stu-id="fffef-125">The `MyProjectName.styles.css` file uses the scope identifier to group a style declaration with its component.</span></span> <span data-ttu-id="fffef-126">Poniższy przykład zawiera styl dla poprzedniego `<h1>` elementu:</span><span class="sxs-lookup"><span data-stu-id="fffef-126">The following example provides the style for the preceding `<h1>` element:</span></span>
...
## <a name="child-component-support"></a><span data-ttu-id="fffef-131">Obsługa składników podrzędnych</span><span class="sxs-lookup"><span data-stu-id="fffef-131">Child component support</span></span>

<span data-ttu-id="fffef-132">Domyślnie izolacja CSS dotyczy tylko składnika, który został skojarzony z formatem `{COMPONENT NAME}.razor.css` , gdzie symbol zastępczy `{COMPONENT NAME}` jest zwykle nazwą składnika.</span><span class="sxs-lookup"><span data-stu-id="fffef-132">By default, CSS isolation only applies to the component you associate with the format `{COMPONENT NAME}.razor.css`, where the placeholder `{COMPONENT NAME}` is usually the component name.</span></span> <span data-ttu-id="fffef-133">Aby zastosować zmiany do składnika podrzędnego, użyj `::deep` Combinator do dowolnego elementu podrzędnego w pliku składnika nadrzędnego `.razor.css` .</span><span class="sxs-lookup"><span data-stu-id="fffef-133">To apply changes to a child component, use the `::deep` combinator to any descendant elements in the parent component's `.razor.css` file.</span></span> <span data-ttu-id="fffef-134">`::deep`Combinator wybiera elementy, które są *elementami podrzędnymi* wygenerowanego identyfikatora zakresu elementu.</span><span class="sxs-lookup"><span data-stu-id="fffef-134">The `::deep` combinator selects elements that are *descendants* of an element's generated scope identifier.</span></span> 

<span data-ttu-id="fffef-135">Poniższy przykład pokazuje składnik nadrzędny o nazwie `Parent` ze składnikiem podrzędnym o nazwie `Child` .</span><span class="sxs-lookup"><span data-stu-id="fffef-135">The following example shows a parent component called `Parent` with a child component called `Child`.</span></span>

<span data-ttu-id="fffef-136">`Parent.razor`:</span><span class="sxs-lookup"><span data-stu-id="fffef-136">`Parent.razor`:</span></span>
...
<span data-ttu-id="fffef-137">`Child.razor`:</span><span class="sxs-lookup"><span data-stu-id="fffef-137">`Child.razor`:</span></span>

```razor
<h1>Child Component</h1>
```

<span data-ttu-id="fffef-138">Zaktualizuj `h1` deklarację w `Parent.razor.css` Combinator, `::deep` Aby wyrównać, że `h1` Deklaracja stylu musi być stosowana do składnika nadrzędnego i jego elementów podrzędnych:</span><span class="sxs-lookup"><span data-stu-id="fffef-138">Update the `h1` declaration in `Parent.razor.css` with the `::deep` combinator to signify the `h1` style declaration must apply to the parent component and its children:</span></span>
...
<span data-ttu-id="fffef-139">`h1`Styl ma teraz zastosowanie do `Parent` składników i `Child` bez konieczności tworzenia ODDZIELNEgo pliku CSS z zakresem dla składnika podrzędnego.</span><span class="sxs-lookup"><span data-stu-id="fffef-139">The `h1` style now applies to the `Parent` and `Child` components without the need to create a separate scoped CSS file for the child component.</span></span>

> [!NOTE]
> <span data-ttu-id="fffef-140">`::deep`Combinator działa tylko z elementami podrzędnymi.</span><span class="sxs-lookup"><span data-stu-id="fffef-140">The `::deep` combinator only works with descendant elements.</span></span> <span data-ttu-id="fffef-141">Następująca struktura HTML stosuje `h1` Style do składników zgodnie z oczekiwaniami:</span><span class="sxs-lookup"><span data-stu-id="fffef-141">The following HTML structure applies the `h1` styles to components as expected:</span></span>
> 
> ```razor
> <div>
>     <h1>Parent</h1>
...
> ```
>
> <span data-ttu-id="fffef-142">W tym scenariuszu ASP.NET Core stosuje identyfikator zakresu składnika nadrzędnego do `div` elementu, dzięki czemu przeglądarka wie o dziedziczeniu stylów ze składnika nadrzędnego.</span><span class="sxs-lookup"><span data-stu-id="fffef-142">In this scenario, ASP.NET Core applies the parent component's scope identifier to the `div` element, so the browser knows to inherit styles from the parent component.</span></span>
>
> <span data-ttu-id="fffef-143">Jednak wykluczenie `div` elementu powoduje usunięcie relacji elementu podrzędnego, a styl **nie** zostanie zastosowany do składnika podrzędnego:</span><span class="sxs-lookup"><span data-stu-id="fffef-143">However, excluding the `div` element removes the descendant relationship, and the style is **not** applied to the child component:</span></span>
>
> ```razor
> <h1>Parent</h1>
>
...
## <a name="css-isolation-configuration"></a><span data-ttu-id="fffef-149">Konfiguracja izolacji CSS</span><span class="sxs-lookup"><span data-stu-id="fffef-149">CSS isolation configuration</span></span>

<span data-ttu-id="fffef-150">Izolacja CSS została zaprojektowana tak, aby działała na zewnątrz, ale zapewnia konfigurację dla niektórych zaawansowanych scenariuszy, takich jak sytuacje, w których istnieją zależności od istniejących narzędzi lub przepływów pracy.</span><span class="sxs-lookup"><span data-stu-id="fffef-150">CSS isolation is designed to work out-of-the-box but provides configuration for some advanced scenarios, such as when there are dependencies on existing tools or workflows.</span></span>

### <a name="customize-scope-identifier-format"></a><span data-ttu-id="fffef-151">Dostosuj format identyfikatora zakresu</span><span class="sxs-lookup"><span data-stu-id="fffef-151">Customize scope identifier format</span></span>

<span data-ttu-id="fffef-152">Domyślnie identyfikatory zakresów używają formatu `b-<10-character-string>` .</span><span class="sxs-lookup"><span data-stu-id="fffef-152">By default, scope identifiers use the format `b-<10-character-string>`.</span></span> <span data-ttu-id="fffef-153">Aby dostosować Format identyfikatora zakresu, zaktualizuj plik projektu do wybranego wzorca:</span><span class="sxs-lookup"><span data-stu-id="fffef-153">To customize the scope identifier format, update the project file to a desired pattern:</span></span>
...
<span data-ttu-id="fffef-156">`scoped.styles.css`Plik jest generowany w katalogu głównym aplikacji.</span><span class="sxs-lookup"><span data-stu-id="fffef-156">The `scoped.styles.css` file is generated at the root of the app.</span></span> <span data-ttu-id="fffef-157">W pliku projektu Użyj `StaticWebAssetBasePath` właściwości, aby zmienić domyślną ścieżkę.</span><span class="sxs-lookup"><span data-stu-id="fffef-157">In the project file, use the `StaticWebAssetBasePath` property to change the default path.</span></span> <span data-ttu-id="fffef-158">Poniższy przykład umieszcza `scoped.styles.css` plik i resztę zasobów aplikacji w `_content` ścieżce:</span><span class="sxs-lookup"><span data-stu-id="fffef-158">The following example places the `scoped.styles.css` file, and the rest of the app's assets, at the `_content` path:</span></span>
...
<span data-ttu-id="fffef-166">W poniższym przykładzie:</span><span class="sxs-lookup"><span data-stu-id="fffef-166">In the following example:</span></span>

* <span data-ttu-id="fffef-167">Ścieżka podstawowa statycznego elementu zawartości sieci Web to `_content/ClassLib` .</span><span class="sxs-lookup"><span data-stu-id="fffef-167">The static web asset base path is `_content/ClassLib`.</span></span>
* <span data-ttu-id="fffef-168">Nazwa zestawu biblioteki klas to `ClassLib` .</span><span class="sxs-lookup"><span data-stu-id="fffef-168">The class library's assembly name is `ClassLib`.</span></span>

```html
<link href="_content/ClassLib/ClassLib.bundle.scp.css" rel="stylesheet">
```

<span data-ttu-id="fffef-169">Aby uzyskać więcej informacji na temat RCLs i bibliotek składników, zobacz:</span><span class="sxs-lookup"><span data-stu-id="fffef-169">For more information on RCLs and component libraries, see:</span></span>

* <xref:razor-pages/ui-class>
* <span data-ttu-id="fffef-170"><xref:blazor/components/class-libraries>.</span><span class="sxs-lookup"><span data-stu-id="fffef-170"><xref:blazor/components/class-libraries>.</span></span>
